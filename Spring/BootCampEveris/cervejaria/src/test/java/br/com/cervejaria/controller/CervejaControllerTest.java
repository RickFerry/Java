...
import static br.com.cervejaria.utils.JsonConvertionUtils.asJsonString;
...
import br.com.cervejaria.builder.CervejaDTOBuilder;
...
public class CervejaControllerTest {
	...
	void whenPOSTIsCalledThenABeerIsCreated() throws Exception {
		...
		mockMvc.perform(post(BEER_API_URL_PATH).contentType(MediaType.APPLICATION_JSON).content(asJsonString(beerDTO)))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.name", is(beerDTO.getNome())))
	...
	void whenGETIsCalledWithoutRegisteredNameThenNotFoundStatusIsReturned() throws Exception {
		...
		// when
		when(beerService.findByName(beerDTO.getNome())).thenThrow(CervejaNotFoundException.class);
	...
	void whenGETListWithBeersIsCalledThenOkStatusIsReturned() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0].name", is(beerDTO.getNome())))
	...
	void whenDELETEIsCalledWithInvalidIdThenNotFoundStatusIsReturned() throws Exception {
		// when
		doThrow(CervejaNotFoundException.class).when(beerService).deleteById(INVALID_BEER_ID);
	...
	void whenPATCHIsCalledToIncrementDiscountThenOKstatusIsReturned() throws Exception {
		QuantidadeDTO quantityDTO = QuantidadeDTO.builder().quantidade(10).build();
		...
		mockMvc.perform(
				MockMvcRequestBuilders.patch(BEER_API_URL_PATH + "/" + VALID_BEER_ID + BEER_API_SUBPATH_INCREMENT_URL)
						.contentType(MediaType.APPLICATION_JSON).content(asJsonString(quantityDTO)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.name", is(beerDTO.getNome())))
				.andExpect(jsonPath("$.brand", is(beerDTO.getMarca())))
				.andExpect(jsonPath("$.type", is(beerDTO.getTipo().toString())))
